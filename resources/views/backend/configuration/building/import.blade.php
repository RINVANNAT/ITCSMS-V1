...
@section ('title', trans('labels.backend.buildings.title') . ' | ' . trans('labels.backend.buildings.sub_import_title'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.buildings.title') }}
        <small>{{ trans('labels.backend.buildings.sub_import_title') }}</small>
    </h1>
@endsection

@section('content')

    {!! Form::open(['route' => 'admin.configuration.buildings.import','id' => 'import_form_building', 'role'=>'form','files' => true])!!}
    <div class="box box-success">
        <div class="box-header with-border">
            <h3 class="box-title">{{ trans('labels.backend.buildings.sub_import_title') }}</h3>
        </div><!-- /.box-header -->
